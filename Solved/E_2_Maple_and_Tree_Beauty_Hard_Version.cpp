...
ostream &operator<<(ostream &dout, pair<T1, T2> &p) {
    return dout << p.ff << ' ' << p.ss << ' ';
}
template <class T> ostream &operator<<(ostream &dout, vector<T> &vec) {
    for (auto i : vec)
        dout << i << ' ';
    return dout;
}
template <class T> void print(T it1, T it2, char sep = ' ') {
...
void CoderAdi27() {
    ...
    vi vec;
    rep(i, 0, mn + 1) vec.pb(freq[i]);
    int r = n - accumulate(all(vec), 0LL);
