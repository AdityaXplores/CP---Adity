...
void solve(){
    ...
    int N, K;
        ...
        long long ans = 0;
        if (K == 2) {
            long long min_even = (long long)4e18, max_even = (long long)-4e18;
            long long min_odd = (long long)4e18, max_odd = (long long)-4e18;
            for (int i = 1; i <= N; ++i) {
                if (i % 2 == 0) {
            ...
            if (min_even <= max_even) ans = max(ans, max_even - min_even);
            if (min_odd <= max_odd) ans = max(ans, max_odd - min_odd);
        } else {
            vector<long long> minp(N + 1), maxp(N + 1);
            minp[1] = maxp[1] = A[1];
            for (int i = 2; i <= N; ++i) {
            ...
            for (int j = 3; j <= N; ++j) {
                ans = max(ans, A[j] - minp[j - 2]);
                ans = max(ans, maxp[j - 2] - A[j]);
            }
        }
        cout << ans << "\n";
}
