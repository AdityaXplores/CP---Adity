...
void solve(int t_){
    ...
    if(t_==2)x++;
    int n=x+y;
    if(x>y||(x==0&&y%2==0)){cout<<"NO\n";return;}
    cout<<"YES\n";
    vec pr,pl,s;
    int nxt=2;
    if(n%2==0){
        pr.pb(1);pl.pb(nxt++);
        fr(i,1,x-1){pr.pb(nxt++);pl.pb(nxt++);}
    }else{
        s.pb(1);
        fr(i,0,x-1){pr.pb(nxt++);pl.pb(nxt++);}
    }
    while(nxt<=n)s.pb(nxt++);
    fr(i,0,x-1)cout<<pr[i]<<" "<<pl[i]<<"\n";
    fr(i,0,x-1)if(pr[i]!=1)cout<<1<<" "<<pr[i]<<"\n";
    for(auto u:s)if(u!=1)cout<<1<<" "<<u<<"\n";
}
int32_t main(){
    #ifndef ONLINE_JUDGE
